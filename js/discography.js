(function() {
  ...
  const renderImages = index => {
    if (index < albums.length) {
      ...
      image.src = `images/discography/${albums[index].art}.jpg`
      ...
      if (pos === -1) {
        ...
        markup +=
         `<div>${track.substring(0, pos)}
  	        <span class="author">${track.substring(pos)}</span>
  	      </div>`
      }
    })
    return markup
    ...
    markup +=
     `<div class="album">
        <div class="image-container">
          <img class="image" id="${index}">
        </div>
        <div class="info-container">
          <div class="block">
            <div class="title">${elem.title}</div>
            <div class="release-date">${elem.release}</div>
          </div>
          <div class="tracklist">${tracklist(elem.tracklist)}</div>
        </div>
      </div>`
  })
